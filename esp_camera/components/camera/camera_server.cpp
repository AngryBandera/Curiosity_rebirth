...
esp_err_t handleRootRequest(httpd_req_t* req) {
    const char* html = 
        ...
        "background:#000;box-shadow:0 4px 8px rgba(0,0,0,0.5);min-height:480px;display:flex;"
        ...
        // Функція для створення елементу картинки стріму
        ...
        "      isStreaming=true;"
        "      container.innerHTML='';"
        "      container.appendChild(createStreamImage());"
        ...
        "    // Ми не чіпаємо стрім, поки не отримаємо фото"
        ...
        "      statusDiv.innerHTML='<span class=\"active\">Photo captured! Resuming stream in 3s...</span>';"
        "      "
        "      // 2. Скачуємо файл"
        ...
        "      link.click();"
        "      "
        "      // 3. АВТО-ВІДНОВЛЕННЯ СТРІМУ"
        "      if(isStreaming) {"
        "         setTimeout(() => {"
        "            statusDiv.innerHTML='<span class=\"active\">Restoring Stream...</span>';"
        "            container.innerHTML='';"
        "            container.appendChild(createStreamImage());"
        "            statusDiv.innerHTML='<span class=\"active\">STREAMING</span>';"
        "         }, 3000);"
